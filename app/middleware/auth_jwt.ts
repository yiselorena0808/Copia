...
export default class AuthJwtMiddleware {
  public async handle({ request, response }: HttpContext, next: () => Promise<void>) {
    ...
    if (!authHeader) return response.unauthorized({ error: 'Falta el token' })
    ...
    try {
      ...
      const id = decoded.id
      const id_empresa = decoded.idEmpresa

      if (!id || !id_empresa) return response.unauthorized({ error: 'Token inválido o incompleto' })
